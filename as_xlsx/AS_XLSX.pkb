...
--
  ...
  is
  begin
    --12.11.2019 ВБРР оптимизировано Сениным Алексеем: логика на exception в разы замедляет выполнение
    if length(g_addtxt2utf8blob_tmp) + length(p_txt) > 32000 then 
...
--
  ...
  is
    ...
    t_comment raw(32767) := utl_raw.cast_to_raw( 'Implementation by Moshkov Roman, as_xlsx18' );
  begin
...
--
  procedure list_validation
    ...
    , p_br_col pls_integer -- bottom right
    , p_br_row pls_integer
    , p_style varchar2 := xl_inf_stop -- stop, warning, information
    , p_title varchar2 := null
    , p_prompt varchar := null
    , p_show_error boolean := false
    , p_error_title varchar2 := null
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="' || workbook.str_cnt || '" uniqueCount="' || workbook.strings.count() || '">'
    ...
    finish_zip( t_excel );
    clear_workbook;
    --insert into test_mra (bblob) values (t_excel);
    return t_excel;
  end;
--
  procedure save
...
--
  ...
  exception
    ...
    then
      ...
      setUseXf( t_useXf );
      return to_blob( utl_raw.cast_to_raw(1));
  end;
--
end;
